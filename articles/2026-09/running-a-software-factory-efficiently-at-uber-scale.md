...
![Weekly active users and agent requests rose sharply from February to August 2026; costs fluctuated but increased overall.](./running-a-software-factory-efficiently-at-uber-scale/01.jpg)
...
![Unit costs for 2026: cost per 1,000 requests drops 34%, cost per session drops 52% from peak to August.](./running-a-software-factory-efficiently-at-uber-scale/02.jpg)
...
![Dashboard categorizing agent types for code generation, validation, deployment, observation, and maintenance tasks.](./running-a-software-factory-efficiently-at-uber-scale/03.jpg)
...
![Formula for total spend: users × sessions/user × turns/session × requests/turn × tokens/request × price/token.](./running-a-software-factory-efficiently-at-uber-scale/04.jpg)
...
-   Per-tool/agent cost, users, and spend share

Where the money is going, and which tool moved
...
Whether the tool is getting cheaper or usage is just shifting
...
-   Cost and cost share
-   Requests and request share
-   Cost per 1,000 requests
-   Cost per 1 million tokens

Which model releases actually changed the bill, at identical or different per-token prices

**Driver decomposition**

Cost change decomposed sequentially into

-   adoption (users)
-   engagement (requests per user)
-   input workload (input tokens per request)
-   output workload (output tokens per request)

Why the number moved, stated exactly, with nothing left in an unexplained residual

**Managed agent outcomes**

For each managed agent: 

-   Outcome-denominated cost (cost per merged PR, cost per review, cost per alert, cost per cleanup);
-   Quality signal (revert rate, F1, MTTR);
-   Volume (diffs landed, reviews posted, alerts triaged)

Whether each managed agent is getting cheaper per unit of value delivered, and whether quality holds through model migrations
...
Benchmark-driven, Pareto-optimal model selection

Model defaults

Tokens / Request

400K context cap & Medium reasoning effort by default

Prompt Caching

Tool search and CLI-resolved MCP calls

Code-mode batching of tool calls

Gateway-routed SaaS MCPs
...
Live cost counter in the status line
...
![Scatter plot comparing cost per review and F1 score for 10 model configurations, highlighting Pareto frontier.](./running-a-software-factory-efficiently-at-uber-scale/05.jpg)
...
![Comparison of 5-minute and 1-hour TTL caching costs for main thread and subagent scenarios, showing cost savings.](./running-a-software-factory-efficiently-at-uber-scale/06.jpg)
...
![Comparison of token usage: installing each server uses ~50-70,000 tokens, tool search plus CLI uses near zero.](./running-a-software-factory-efficiently-at-uber-scale/07.jpg)
...
![Comparison of MCP tool use and code-mode workflows, highlighting model turns, token usage, and data flow steps.](./running-a-software-factory-efficiently-at-uber-scale/08.jpg)
...
Tokens per query, measured in the same Claude Code session.
...
![Single shell command accesses multiple SaaS tools via MCP Gateway, including workspace, project, messaging, and design.](./running-a-software-factory-efficiently-at-uber-scale/09.jpg)
...
![Query with the graph took 38s and was correct; without the graph took 20m 09s and was wrong.](./running-a-software-factory-efficiently-at-uber-scale/10.jpg)
...
![Terminal-style status bar with colorful metrics, token usage, costs, and a URL, labeled 'What an engineer sees, all the time'.](./running-a-software-factory-efficiently-at-uber-scale/11.jpg)
...
Built directly into the runtime, it requires zero setup or opt-in. Executing the _cost dashboard_ skill analyzes all session traces for the user across local and remote cloud sandboxes across all harnesses they use. Rather than producing an aggregate metric, it flags 16 distinct anti-patterns across sessions, pairing each with its financial impact and a targeted remediation. Some of the categories include:
...
![Dashboard showing $4162.07 total spend, 433 sessions, 95% cache hit rate, $1097.82 miss cost, $1213.87 savings.](./running-a-software-factory-efficiently-at-uber-scale/12.jpg)
...
This is a collective effort by many engineers who are building the most efficient blocks to implement Software Factory at Uber scale, while ensuring we get ROI for every token we spend. We would like to thank our core team involved in various efforts across Software Factory listed here: Abhishek Bhatia, Adam Huda, Aditya Patel, Alok Srivastava, Ameya Ketkar, Anil Purohit, Atakan Kandemir, Ben Chou, Brandon Barker, Danielle Yim, Deepanshu Mehndiratta, Gaurav Gill, Israel Marban, Jason Varbedian, Karen Xu, Lei Shi, Mager Mager, Meghana Somasundara, Peng Liu, Preet Inder, Qiushen Wang, Rush Tehrani, Shesh Patel, Shiven Tripathi, Shubham Gupta, Stas Khalup, Ting Chen, Tse-Shi Wang, Ty Smith, Vikram Hullukunte, Viv Keswani, Weiqiang Wang, Will Bond.

Also would like to thank Johannes Gehrke, Mattie Toia, Sumanth Sukumar, and Praveen Neppalli Naga for their leadership.

Cover Photo Attribution: Himer Romana

_Anthropic® is a registered trademark of Anthropic PBC._ 

_Claude Code™ and Claude® are trademarks of Anthropic, PBC._
