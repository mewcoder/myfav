...
describe('repository data files', () => {
  it('retain all records and satisfy the production contract', async () => {
    const [sites, repos, articles, notes] = await Promise.all(['sites', 'repos', 'articles', 'notes'].map(async (name) => JSON.parse(await readFile(new URL(`../../public/data/${name}.json`, import.meta.url), 'utf8'))))
    ...
    expect(validateContent({ sites, repos, articles, notes })).toEqual([])
  })
